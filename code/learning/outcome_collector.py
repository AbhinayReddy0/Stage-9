...
Also upserts `adaptive_quantile_state` — per-(SKU, model, pattern, horizon)
coverage stats derived from the last COVERAGE_LOOKBACK_DAYS of outcomes.
This is how Stage 9 discovers whether its claimed p90 confidence is real.
...
# Rolling window used to compute quantile coverage in adaptive_quantile_state.
...
# ---------------------------------------------------------------------------
# Adaptive quantile state
# ---------------------------------------------------------------------------

_FETCH_OUTCOMES_FOR_COVERAGE = """
    SELECT
        fo.tenant_id,
        fo.sku_id,
        fo.assigned_model,
        ...
        fo.forecast_value,
        f.selected_quantile
    FROM stage9.forecast_outcomes fo
    JOIN stage9.forecasts f
        ON  f.tenant_id = fo.tenant_id
        AND f.sku_id    = fo.sku_id
        AND f.run_id    = fo.run_id
    WHERE fo.tenant_id    = %(tenant_id)s
      AND fo.outcome_date >= CURRENT_DATE - %(lookback)s::int
      AND fo.actual_value   IS NOT NULL
      AND fo.forecast_value IS NOT NULL
      AND f.pattern_label   IS NOT NULL;
"""

_UPSERT_AQS_ROW = """
    INSERT INTO stage9.adaptive_quantile_state
        (tenant_id, sku_id, assigned_model, pattern_label, horizon_days,
         backtest_window_days, sample_size, target_quantile, actual_coverage,
         calibration_gap, last_updated)
    VALUES (
        %(tenant_id)s, %(sku_id)s, %(assigned_model)s, %(pattern_label)s,
        %(horizon_days)s, %(backtest_window_days)s, %(sample_size)s,
        %(target_quantile)s, %(actual_coverage)s, %(calibration_gap)s, NOW()
    )
    ON CONFLICT (tenant_id, sku_id, assigned_model, pattern_label, horizon_days)
    DO UPDATE SET
        backtest_window_days = EXCLUDED.backtest_window_days,
...
def _fetch_outcomes_for_coverage(conn, tenant_id: str) -> list[dict]:
    """
    Fetch the raw outcome rows needed to compute quantile coverage.
    Returns one dict per forecast_outcomes row within COVERAGE_LOOKBACK_DAYS.
    """
    cur = conn.cursor()
    try:
        cur.execute(
            _FETCH_OUTCOMES_FOR_COVERAGE,
            {"tenant_id": tenant_id, "lookback": COVERAGE_LOOKBACK_DAYS},
        )
        cols = [d[0] for d in cur.description]
        return [dict(zip(cols, row)) for row in cur.fetchall()]
    finally:
...
def _compute_coverage_stats(rows: list[dict]) -> list[dict]:
    """
    Group outcome rows by (tenant_id, sku_id, assigned_model, pattern_label,
    horizon_days) and compute per-group coverage statistics.

    For each group:
        sample_size     — number of closed outcomes in the lookback window
        target_quantile — average selected_quantile across runs (stable in
                          practice; averaging handles the rare mid-period change)
        actual_coverage — fraction of outcomes where actual_value <= forecast_value
        calibration_gap — actual_coverage - target_quantile
                          negative → over-confident (actuals exceed forecast
                          more often than the quantile implies)
                          positive → under-confident (forecast is too conservative)
    ...
    stats = []
    for (tenant_id, sku_id, model, pattern, horizon), group in groups.items():
        sample_size     = len(group)
        covered         = sum(1 for r in group if r["actual_value"] <= r["forecast_value"])
        avg_quantile    = sum(float(r["selected_quantile"]) for r in group) / sample_size

        actual_coverage = round(covered / sample_size, 3)
        target_quantile = round(avg_quantile, 3)
        calibration_gap = round(actual_coverage - target_quantile, 4)
        ...
        stats.append({
            ...
            "backtest_window_days": COVERAGE_LOOKBACK_DAYS,
...
def _upsert_aqs_row(conn, row: dict) -> None:
    """Write one adaptive_quantile_state row. Idempotent via ON CONFLICT DO UPDATE."""
    cur = conn.cursor()
    try:
        cur.execute(_UPSERT_AQS_ROW, row)
    finally:
        cur.close()


def _update_adaptive_quantile_state(conn, tenant_id: str) -> int:
    """
    Fetch raw outcomes, compute coverage stats in Python, upsert results.
    Returns the number of rows written.
    """
    rows = _fetch_outcomes_for_coverage(conn, tenant_id)
    if not rows:
        return 0
    stats = _compute_coverage_stats(rows)
    for row in stats:
        _upsert_aqs_row(conn, row)
    return len(stats)
...
def run_for_tenant(tenant_id: str, conn) -> dict:
        ...
        Summary dict:
            {
                ...
                "total_errors": int,
                "aqs_rows_upserted": int,  -- adaptive_quantile_state rows written;
                                              0 on failure (already logged)
            }
    ...
    # Update adaptive_quantile_state from the outcomes written this run.
    # forecast_outcomes rows are already committed above; this is a separate
    # transaction so a failure here never rolls back the outcome writes.
    aqs_rows = 0
    try:
        aqs_rows = _update_adaptive_quantile_state(conn, tenant_id)
        conn.commit()
        logger.info("tenant=%s: adaptive_quantile_state upserted %d rows", tenant_id, aqs_rows)
    except Exception:
        logger.exception(
            "tenant=%s: adaptive_quantile_state update failed — "
            "forecast_outcomes already committed, coverage stats not updated",
            tenant_id,
        )
        try:
            conn.rollback()
    ...
    summary = {
        ...
        "aqs_rows_upserted": aqs_rows,
    }
    logger.info(
        "tenant=%s: OutcomeCollector finished — written=%d skipped=%d errors=%d "
        "aqs_rows=%d duration=%.2fs",
        tenant_id, total_written, total_skipped, total_errors,
        aqs_rows, summary["duration_seconds"],
    )
    return summary
